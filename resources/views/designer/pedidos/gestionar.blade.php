...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        {{-- RUTA AJUSTADA: De admin.pedidos.index a designer.pedidos.index --}}
        <a href="{{ route('designer.pedidos.index') }}" class="btn btn-outline-secondary">
            <i class="bi bi-arrow-left me-2"></i>Volver al Listado
        </a>
    </div>

    {{-- SECCIÓN DE ESTADO ACTUAL Y ASIGNACIÓN (MANTENIDA) --}}
    <div class="row mb-5 g-4">
        ...
        {{-- CARD 3: DISEÑADOR ASIGNADO (MANTENIDA) --}}
        <div class="col-md-4">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-4">
            ...
            {{-- Descripción del Pedido y Detalles (MANTENIDA) --}}
        ...
        {{-- COLUMNA DERECHA: Historial de Movimientos (TIME-LINE) (MANTENIDA) --}}
        <div class="col-lg-8">
                                ...
                                <div class="timeline-content flex-grow-1">
                                    ...
                                    @if(!empty($evento['hisImagen']))
                                        @php
                                            ...
                                            // Asume que tu API está en la misma red o usa un proxy Laravel si es necesario.
                                            // $urlImagen = route('designer.pedidos.imagen-proxy', ['path' => $rutaImagen]); // Si implementas un proxy
...
<script>
    ...
    function actualizarEstadoPedido(event, pedidoId) {
        ...
        // 🛠️ CORRECCIÓN: INCLUIR EL TOKEN Y EL MÉTODO OVERRIDE EN EL BODY DE FormData
        ...
        // La URL de la ruta es: /designer/pedidos/{id}/estado-historial (es un POST que simula PATCH)
        fetch(`/designer/pedidos/${pedidoId}/estado-historial`, { 
            method: 'POST', 
            // Ya no necesitamos pasar X-CSRF-TOKEN ni X-HTTP-Method-Override en headers
            body: formData
        })
        .then(response => {
             // 🛡️ Manejo mejorado de errores del servidor
             if (response.status === 419) {
                throw new Error('Sesión Expirada o Error de Seguridad (CSRF). Por favor, refresque la página.');
             }
             if (!response.ok) {
                 // Intentamos obtener el JSON de error
                 return response.json().then(data => ({ status: response.status, body: data }));
             }
             // Si es 2xx, obtenemos el JSON.
             return response.json().then(data => ({ status: response.status, body: data }));
        })
        .then(({ status, body }) => {
            if (status >= 400 && status !== 419) throw new Error(body.message || 'Error en el servidor API');
